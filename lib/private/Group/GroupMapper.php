...
class GroupMapper extends Mapper {
	...
	/**
	 * Creates a backend group from attributes' array.
	 * @param array $row the set of attributes which should be converted to a backend group
	 * @return BackendGroup
	 * @since 10.0.0
	 */
	public function mapToGroup($row) {
		/** @var $group BackendGroup */
		$group = parent::mapRowToEntity($row);
		return $group;
	}
}
